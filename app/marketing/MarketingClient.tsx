...
function CampaignTableControls({
  search,
  onSearchChange,
  onExport,
}: {
...
function CampaignRow({ c, isLast }: { c: Campaign; isLast: boolean }) {
  const ss = STATUS_STYLE[c.status];
  return (
    <tr
      style={{
        borderBottom: isLast ? undefined : "1px solid #1F2937",
    ...
    >
      ...
      <td
        ...
        style={{ color: c.cpa <= 1 ? "#10B981" : c.cpa <= 5 ? "#9A7030" : "#9CA3AF" }}
      >
        ${c.cpa.toFixed(2)}
      </td>
      <td className="py-3 px-3">
  ...
  return (
    ...
    >
      <p className="text-[11px]" style={{ color: "#6B7280" }}>
        Showing {filteredCount} of {CAMPAIGNS.length} campaigns
        {search && (
          <span
            ...
            role="button"
            tabIndex={0}
            onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") onClearSearch(); }}
          >
            Clear filter
          </span>
        )}
...
function CampaignTable() {
  ...
  const filtered = useMemo(() => {
    ...
    if (sortField !== "none") {
      rows.sort((a, b) => {
        const av = sortField === "cpa" ? a.cpa : sortField === "roas" ? a.roas
          : sortField === "budget" ? a.budget : sortField === "conv" ? a.conv : a.impressionsRaw;
        const bv = sortField === "cpa" ? b.cpa : sortField === "roas" ? b.roas
          : sortField === "budget" ? b.budget : sortField === "conv" ? b.conv : b.impressionsRaw;
  ...
  return (
    <div className="rounded-xl p-6" style={{ backgroundColor: "#111827", border: "1px solid #1F2937" }}>
      <CampaignTableControls search={search} onSearchChange={setSearch} onExport={exportCSV} />
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <CampaignTableHead sortField={sortField} sortDir={sortDir} onSort={handleSort} />
          <tbody>
            {filtered.map((c, i) => (
              <CampaignRow key={c.name} c={c} isLast={i === filtered.length - 1} />
            ))}
          </tbody>
        </table>
      </div>
      <CampaignTableFooter filteredCount={filtered.length} search={search} onClearSearch={() => setSearch("")} />
    </div>
  );
}
