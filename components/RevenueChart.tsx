...
const streams = [
  ...
  { name: "Merchandise",          value: 0.2, pct: 1,  color: "#7C6FFF", mom: -0.02 },
];

interface TProps { active?: boolean; payload?: { name: string; value: number; payload: typeof streams[0] }[]; }
function Tip({ active, payload }: TProps) {
  ...
  return (
    <div style={{ background: "#0C1220", border: "1px solid #243044", borderRadius: 8, padding: "10px 14px", fontSize: 12, minWidth: 160 }}>
      ...
      <p style={{ color: "#8B97AA" }}>Revenue: <strong style={{ color: "#E8ECF4" }}>${d.value}M</strong></p>
      <p style={{ color: "#8B97AA" }}>Share: <strong style={{ color: d.color }}>{d.pct}%</strong></p>
      <p style={{ color: "#8B97AA" }}>MoM: <strong style={{ color: d.mom >= 0 ? "#00C896" : "#FF4F5B" }}>{d.mom >= 0 ? "+" : ""}${d.mom.toFixed(2)}M</strong></p>
    </div>
  );
}
  ...
  return (
    <div className="rounded-[10px] p-5 h-full" style={{ background: "#0C1220", border: "1px solid #1A2437" }}>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 style={{ fontSize: 15, fontWeight: 700, color: "#E8ECF4" }}>Revenue Breakdown</h2>
          <p style={{ fontSize: 11, color: "#4E5E74", marginTop: 2 }}>All streams · Q1 2026 · Feb–Mar</p>
        </div>
        <span style={{ fontSize: 9, fontWeight: 800, padding: "2px 7px", borderRadius: 4, textTransform: "uppercase", letterSpacing: "0.08em", background: "rgba(255,153,0,0.1)", color: "#FF9900", border: "1px solid rgba(255,153,0,0.2)" }}>Q1 2026</span>
      </div>

      {/* Donut — absolute overlay for center label */}
      <div className="relative" style={{ height: 200 }}>
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            <Pie data={streams} cx="50%" cy="50%" innerRadius={64} outerRadius={90}
              paddingAngle={3} dataKey="value" strokeWidth={0} startAngle={90} endAngle={-270}>
              {streams.map(d => <Cell key={d.name} fill={d.color} opacity={0.9} />)}
            </Pie>
            <Tooltip content={<Tip />} />
          </PieChart>
        </ResponsiveContainer>
        {/* Absolute overlay — never use SVG children for center labels in recharts */}
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          <span style={{ fontSize: 24, fontWeight: 900, color: "#E8ECF4", lineHeight: 1, fontVariantNumeric: "tabular-nums", letterSpacing: "-0.03em" }}>${total}M</span>
          <span style={{ fontSize: 10, color: "#4E5E74", marginTop: 3 }}>Total Revenue</span>
        </div>
      </div>
      ...
      {/* MoM */}
      <div style={{ borderTop: "1px solid #1A2437", marginTop: 16, paddingTop: 14 }}>
        ...
        <div className="grid grid-cols-2 gap-1.5">
          {streams.map(s => (
            <div key={s.name} className="flex items-center justify-between px-2.5 py-1.5 rounded-lg" style={{ background: "#060A12" }}>
