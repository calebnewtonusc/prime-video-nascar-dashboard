const platforms = [
  { name: "Prime Video",   share: 38, growth:  8, subs: "16.4M", color: "#3A6FA8", isPrimary: true  },
  ...
  return (
    <div className="card-hover rounded-[10px] overflow-hidden" style={{ background: "#0C1220", border: "1px solid #1A2437" }}>
      <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: "1px solid #1A2437" }}>
        ...
        <span style={{ fontSize: 9, fontWeight: 800, padding: "2px 7px", borderRadius: 3, textTransform: "uppercase", letterSpacing: "0.07em", background: "rgba(0,168,255,0.1)", color: "#3A6FA8", border: "1px solid rgba(0,168,255,0.2)" }}>
          #1 Streamer
        </span>
      </div>
            ...
            <div style={{ flex: 1 }}>
              <div className="flex items-center justify-between mb-1">
                <span style={{ fontSize: 11, fontWeight: p.isPrimary ? 700 : 500, color: p.isPrimary ? "#E8ECF4" : "#8B97AA" }}>
                  {p.name}
                  {p.isPrimary && <span style={{ marginLeft: 5, fontSize: 8, fontWeight: 800, color: "#3A6FA8" }}>YOU</span>}
                </span>
                <div className="flex items-center gap-2">
                  <span style={{ fontSize: 11, fontWeight: 700, color: p.isPrimary ? "#3A6FA8" : "#8B97AA", fontVariantNumeric: "tabular-nums" }}>{p.share}%</span>
              ...
              <div style={{ height: 3, background: "#1A2437", borderRadius: 2, overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${(p.share / 40) * 100}%`, background: p.isPrimary ? "#3A6FA8" : "#243044", borderRadius: 2 }} />
              </div>
            </div>
          </div>
        ))}
